...
public class SQLLink {
	...
	static final String DB_URL = "jdbc:mysql://stormboomer.de/dr_mcserver";
	
	static final String USER = SQLConfig.getUSER();
	protected static final String PASS = SQLConfig.getPASS();//C5ep1XQpetYP3d1e9wcu
	...
	public static ArrayList<List<String>> getRatingValues(String name) {
		...
		try {
			...
			Bukkit.getLogger().info("Init for JDBC_DRIVER");
			...
			Bukkit.getLogger().warning(se.getStackTrace().toString() + " SQLException");
		} catch (Exception e) {
			Bukkit.getLogger().warning(e.getStackTrace().toString() + " Exception");
		} finally {
			try {
				if (stmt != null) {
					stmt.close();
